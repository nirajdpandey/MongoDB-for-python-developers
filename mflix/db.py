...
def get_movies_by_country(countries):
    ...
    try:
        ...
        # Use projection in find class instead of in query itself
        # Otherwise it won't work and won't be easy to write as python dict
...
get_movies_by_country(['Russia', 'Japan'])
...
def build_query_sort_project(filters):
    ...
    if filters:
        if "text" in filters:
            ...
            # TODO: Text and Subfield Search: Done
...
def login_user(email, jwt):
    ...
    try:
        ...
        # run these below lined in mongo shell in case you get any error.
        # db.users.deleteMany({})
